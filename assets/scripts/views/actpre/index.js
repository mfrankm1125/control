 ...
 */
var titles={'addtitle':"Nueva Actividad Presupuestaria" ,'updatetitle':"Editar Actividad Presupuestaria"};
var isEdit=null,idEdit=null;
$(document).ready(function () {

    var rowSelection = $('#tabla_grid').DataTable({
        "ajax": url_base+'actpre/getDataTable',
        "columns": [
            { "data": null },
            ...
            {  sortable: false,
                "render": function ( data, type, full, meta ) {
                    var id = full.id_actpresupuestariareal;
                    html="";
...
function btnSave(id){
    ...
    if(bol){
        ...
        $.post(url_base+'actpre/setForm',dataForm,function (data) {
            console.log(data);
            if(data === "ok"){
                status=true;
...
function  editar(id){
    ...
    var idd=parseInt(id);
    $.post(url_base+'actpre/getData','id='+idd,function (data) {
            console.log(data);
            setForm(data[0]);
...
function setForm(data){
    ...
    if(valData > 0){
        $("#name").val(data.nombre);
         $("#isEdit").val(1);
        $("#idEdit").val(data.id_actpresupuestariareal);
    }else{
        alert_error("Error");
    }
}
...
function eliminar(id) {
    if(confirm('¿Esta seguro de eliminar este registro?')){
        var idd=parseInt(id);
        $.post(url_base+'actpre/deleteData','id='+idd,function (data) {
            console.log(data);
