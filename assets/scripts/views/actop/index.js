...
$(document).ready(function () {
    ...
    var rowSelection = $('#tabla_grid').DataTable({
        "ajax": url_base+'actop/getDataTable',
        "columns": [
            { "data": null },
            ...
            {  sortable: false,
                "render": function ( data, type, full, meta ) {
                    var id = full.id_actvpresupuestaria;
                    html="";
...
function btnSave(id){
    ...
    if(bol){
        ...
        $.post(url_base+'actop/setForm',dataForm,function (data) {
            console.log(data);
            if(data === "ok"){
                status=true;
...
function  editar(id){
    ...
    var idd=parseInt(id);
    $.post(url_base+'actop/getData','id='+idd,function (data) {
            console.log(data);
            setForm(data[0]);
...
function setForm(data){
    ...
    if(valData > 0){
        $("#name").val(data.nombre);
         $("#isEdit").val(1);
        $("#idEdit").val(data.id_actvpresupuestaria);
    }else{
        alert_error("Error");
    }
}
...
function eliminar(id) {
    if(confirm('¿Esta seguro de eliminar este registro?')){
        var idd=parseInt(id);
        $.post(url_base+'actop/deleteData','id='+idd,function (data) {
            console.log(data);
