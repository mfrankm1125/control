 ...
 */
var titles={'addtitle':"Agregar Almacen" ,'updatetitle':"Editar Almacen"};
var isEdit=null,idEdit=null;
$(document).ready(function () {

    var rowSelection = $('#tabla_grid').DataTable({
        "ajax": url_base+'almacen/getDataTable',
        "columns": [
            { "data": null },
            ...
            { "data": 'direccion' },
            { "render": function ( data, type, full, meta ) {
                var date = full.fechacrea;
                var d = new Date(date);
                var html=""+formatDate(d,0);
                //var lastname = full.apellidos;
...
function btnSave(id){
    ...
    console.log($('#formAlmacen').serialize());

    //bol=false;
    if(bol){

        var dataForm=$('#formAlmacen').serialize();
        $.post(url_base+'almacen/setForm',dataForm,function (data) {
            console.log(data);
            if(data === "ok"){
                status=true;
...
function  editar(id){
    ...
    var idd=parseInt(id);
    $.post(url_base+'almacen/getData','id='+idd,function (data) {
            console.log(data);
...
function eliminar(id) {
    if(confirm('¿Esta seguro?')){
        var idd=parseInt(id);
        $.post(url_base+'almacen/deleteData','id='+idd,function (data) {
            console.log(data);
