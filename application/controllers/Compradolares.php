        ...
        if(sizeof($post)>0){
            ...
            if($isEdit == 0){
                ...
                foreach($detallecompra as $k=>$i ){
                    $ddddxxxxx=explode("-",$i["selProducto"]);
                   // echo $this->showErrors($ddddxxxxx);
                    $iddetcompra_=$ddddxxxxx[0];
                    $idcompra_=$ddddxxxxx[1];
                    ...
                    if($iddetcompra_ == 0){
                        ...
                        $this->db->set("montocompra", "montocompra+".$i['cantcompra'] , FALSE);
                        $this->db->where('iddetcomprad',$iddetcompra_ );
                        $this->db->update('detcomprad'); // gives UPDATE mytable SET field = field+1 WHERE id = 2
                        $idMax=$idcompra_;
                    }
...
detcomprad.montocompra-(
    ...
    and detventad.iddetproductod=detcomprad.iddetcomprad  
    ...
    where comprad.estado>0 and detcomprad.estado>0 
...
where stock> 0";
        ...
        $q2="SELECT dtf.*, 
                (dtf.montocompra + dtf.montohijos)-(
                    ...
                    INNER JOIN detcomprad as dtc ON dtc.iddetcomprad = detventad.iddetproductod 
                    where detventad.estado > 0 and ventad.estado>0 and dtc.estado>0
                    and detventad.iddetproductod=dtf.iddetcomprad  
                ...
                proveedor.nombre,
                proveedor.idproveedor, 
                COALESCE(sum(ph.montocompra),0) as montohijos ,
                detcomprad.idpadre
                FROM
                    comprad
                    INNER JOIN detcomprad ON comprad.idcomprad = detcomprad.idcomprad
                    left JOIN detcomprad as ph ON detcomprad.iddetcomprad = ph.idpadre 
                    INNER JOIN moneda ON moneda.idmoneda = detcomprad.idmoneda
                    INNER JOIN proveedor ON proveedor.idproveedor = comprad.idproveedor
                where comprad.estado>0 and detcomprad.estado>0 and COALESCE(detcomprad.idpadre,0)=0 
                GROUP BY detcomprad.iddetcomprad 
                ) as dtf
