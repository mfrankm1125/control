...
function verDetalleVentaX(id,this_){
    ...
    $.post(url_base+"ventas/getDetVenta_",{idventa:id},function (data) {
        var arrVenta=data;
        var sumx=0;
        var ht="";
        var totVentaSoles=0;
        ...
        $.each(arrVenta,function (kx,ix) {
            ...
            ht+="<p style='margin-bottom: 0px;'> "+ix.montobtc+" <i class='fa fa-bitcoin'></i> =  S/ "+ix.montosoles+"</p>";

        });
        ht+="<b style='margin-bottom: 0px;' >Total  "+parseFloat(parseFloat(totVentaBtc).toFixed(8))+" <i class='fa fa-bitcoin'></i>  = S/ "+ parseFloat(totVentaSoles).toFixed(2)+" </b>";
        div.html(ht);
    },'json');
  ...
  //  console.log(td);
    ...
    function verDetallePagoX(id,this_){
        ...
        $.post(url_base+"ventas/getDetPago_",{idventa:id},function (data) {
            var arrPago=data;
            var totalACobrar=0;
...
function testxxxx() {
    $.post(url_base+"ventas/datax",{},function (data) {
...
function calcResto(){
  ...
  $(document).on("click","#btnSaveForm",function () {
      ...
      btn.button("loading");
      $.post(url_base+"ventas/setForm",form,function (data) {
         if(data.status == "ok"){
             refrescar();
  ...
  function eliminar(id) {
      ...
      $.post(url_base+"ventas/delete",{"id":id},function (data) {
          if(data.status == "ok"){
              refrescar();
              alert_success("Correcto");
  ...
  function loadTrForTbody(onlyData) {
     var xd=$.ajax({
          async: false,
          type: 'post',
          url: url_base+'ventas/getProdStock',
          data:{"id":1},
          dataType:"json",
  ...
  $(document).on("change","select[name='prodVenta[]']",function () {
      ...
      var pcomision=tr.find("input[name='pcomision[]']").val("0");

      var pvcom=tr.find(".pvcom").html(pventa);
    ...
    $(document).on("keyup","input[name='cantBtc[]']",function () {
        ...
        var inputPBaseventaComision=parseFloat(tr.find(".pvcom").html());
        inputPBaseventa=inputPBaseventaComision;
        var ssstotal=0;
    ...
    $(document).on("click","#btnGenerarDtVenta",function () {
        ...
        $.post(url_base+"ventas/reporte",{fechaini:fechaini,fechaend:fechaend},function (data) {
            if(data.status == "ok"){
    ...
    function editar(id,data) {
        ...
        $("#isEdit").val(1);
        $("#idEdit").val(id);
        $.post(url_base+"ventas/getDataDetalleVenta",{id:id},function (data) {
            if(data.status =="ok"){
    ...
    $(document).on("change","#idreffecha",function () {
        ...
        $.post(url_base+"ventas/validarIdRef",{"id":vl},function (data) {
            if(data.status == "ok"){
                if(data.data.c == "0" ){
    ...
    $(document).on("change","input[name='nrooperacion[]']",function () {
       ...
       $.post(url_base+"ventas/validarNroOperacion",{"id":vl},function (data) {
            if(data.status == "ok"){
                if(data.data.c == "0" ){
...
$(document).on("click","#btnGenerarDtVentaXBanco",function () {
    var fechaini=$("#dtfechaventaini").val();
    var fechaend=$("#dtfechaventaend").val();
    $.post(url_base+"ventas/reporteByBanco",{fechaini:fechaini,fechaend:fechaend},function (data) {
        if(data.status == "ok"){
    ...
    $(document).on("click","#btnSaveFormCliente",function () {
        ...
        $.post(url_base+"ventas/setFormCliente",form,function (data) {
            if(data.status == "ok"){
                refrescar();
                alert_success("Correcto");
    ...
    $(document).on("change","#cliente",function () {
        ...
        $.post(url_base+"ventas/searchVenta",{idcliente:idcliente,fecha:$("#fecha").val()},function(data){
...
function verDetalleReporteBanco(thisx,data) {
    ...
    $(document).on("click","#btnSaveEnviarACaja",function () {
        ...
        $.post(url_base+"ventas/enviocaja",form,function(data){
            if(data.status == "ok"){
    ...
    $(document).on("click","#btnGenerarDtVentaXCliente",function () {
        ...
        $.post(url_base+"ventas/reportedeventaporcliente",{fechaini:fechaini,fechaend:fechaend},function(data){
            console.log(data);
            if(data.status == "ok"){
    ...
    function updateTableBancos(moneda) {
        ...
        $.post(url_base+"ventas/getXupdateListBancos",{moneda:moneda},function (data) {
            if(data.status=="ok"){
 ...
 ///--------------------------
    ...
    $(document).on("click","#btnGenerarDtVentaXProveedor",function () {
        ...
        $.post(url_base+"ventas/reporteByProveedores",{fechaini:fechaini,fechaend:fechaend},function (data) {
            if(data.status == "ok"){
    ...
    $(document).on("click","#btnGeneraComprobante",function () {
        ...
        btn.button("loading");
        $.post(url_base+"ventas/setNewCompro",formCom+"&textRuc="+rucG,function (r) {
            if(r.status){
    ...
    $(document).on("keyup","input[name='pcomision[]']",function (){
        var pv=$(this) ;
        var valx=pv.val();
        var tr=pv.closest("tr");
        var pventa=tr.find("input[name='pventa[]']");
        var cantBtc=tr.find("input[name='cantBtc[]']");
        var pvcom=tr.find(".pvcom");
        var cantSoles=tr.find("input[name='cantSoles[]']");
        var subtx=tr.find("input[name='subTotal[]']");
        let ppvcom=pventa.val()-(pventa.val()*valx/100);
        pvcom.html(ppvcom);

        let cantSolx=ppvcom*cantBtc.val();
        cantSoles.val(cantSolx);
        subtx.val(cantSolx*cantBtc.val());
        calcTotalDetalleTable();
    });
    ...
    <div class="table-responsive">
        ...
        <table class="table table-striped" id="tableReporteVentasxProveedores">
            ...
            <tr>
                ...
                <th><i class="fa fa-btc"></i> Vendido</th>
                <th>S/ x venta</th>
                <th>Precio venta</th>
            </tr>
            </thead>
                            ...
                            <div class="table-responsive">
                                <table class="table   table-hover">
                                    ...
                                    <tr>
                                        <th>IDPBTC</th>
                                        <th>Stock</th>
                                        <th>Precio venta/FEE*</th>
                                        <th>Cant.Venta BTC</th>
                                        <th>Cant.Venta S/</th>
                                        <th>SubTotal</th>
                                        <th>Acción</th>
                                    </tr>
    ...
    <tr>
        <td>
            <select name="prodVenta[]" class="form-control" >
                ...
                <% _.each(data,function(i,k){  %>
                <option value="<%=i.iddetcompra%>-<%=parseFloat(i.stock)%>-<%=parseFloat(i.preciobaseventa)%>-<%=parseFloat(i.preciounidadbtc)%>"  ><%=i.proveedor%> (<%=(parseFloat(i.stock))%> BTC )</option>
                <% }); %>
            </select>
        ...
        <td>
            <input type="text" class="form-control" name="pventa[]"   style="width: 80px;text-align: right;display: inline-block;" >
            <input type="text" class="form-control" name="pcomision[]" value="0"   style="width: 40px;text-align: right;display: inline-block;padding-left: 2px;padding-right: 2px;" >=<span class="pvcom"></span> <br>
...
<script type="text/template" id="tmpTrTbodyDetalleVentaEdit" >
 <% _.each(data,function(i,k){  %>
     <tr>
         <td>
             <select name="prodVenta[]" class="form-control" >
                 ...
                 valCompraStock=parseFloat(ii.preciounidadbtc);
                   %>
                 <option selected="selected" value="<%=ii.iddetcompra%>-<%=parseFloat(ii.stock)%>-<%=parseFloat(ii.preciobaseventa)%>-<%=parseFloat(ii.preciounidadbtc)%>"  ><%=ii.proveedor%> (<%=(parseFloat(ii.stock))%> BTC )</option>
                 ...
                 %>
                 <option value="<%=ii.iddetcompra%>-<%=parseFloat(ii.stock)%>-<%=parseFloat(ii.preciobaseventa)%>-<%=parseFloat(ii.preciounidadbtc)%>"  ><%=ii.proveedor%> (<%=(parseFloat(ii.stock))%> BTC )</option>
                 ...
                 %>
                 <option selected="selected"  value="<%=i.idrefdetcompra%>-<%=i.montobtc%>-<%=i.preciobaseventa%>-<%=parseFloat(i.preciounidadbtccomprado)%>" ><%=i.proveedor%> (<%=i.montobtc%> BTC) </option>
                 <%  }   %>
             </select>
         ...
         <td>
                <%
                    var pvcomx_=i.precioventa - (i.precioventa*i.comision/100);

                %>
             <input type="text" class="form-control" name="pventa[]" value="<%=i.precioventa%>"   style="width: 80px;text-align: right;display: inline-block" >

             <input type="text" class="form-control" name="pcomision[]" value="<%=i.comision%>"   style="width: 40px;text-align: right;display: inline-block;padding-left: 2px;padding-right: 2px;" >=<span class="pvcom"><%=pvcomx_%></span> <br>
         ...
         <% var ssstotal= (parseFloat(i.montobtc) * parseFloat(pvcomx_));
    ...
    <div class="table-responsive">
        ...
        <table class="table table-striped" id="tableReporteVentas">
             ...
             _.each(data,function(i,k){ %>
                <tr>
                    ...
                    <td>
                        ...
                        %>
                        <p style="margin-bottom: 0px;" > <%=ii.montobtc%><i class="fa fa-bitcoin"></i> = S/<%=ii.montosoles%>    / <%=ii.preciounidadbtccomprado%> </p>
                        <% }); %>
                        <b>Total:<%= ttbtc%><i class="fa fa-bitcoin"></i> = S/<%= ttsoles%>   </b>
                    ...
                    <td>
                        ...
                        totalBtcVendido+=parseFloat(ix.montobtc);
                         %>
                        <p><%=parseFloat(ix.montobtc)%>BTC = Venta:<%=ix.precioventa%> Compra: <%=ix.preciounidadbtccomprado%>
            ...
            <tr>
                ...
                <th> </th>
                <th> </th>
                <th><%= parseFloat(parseFloat(totalBtcVendido).toFixed(8)) %> BTC  </th>
