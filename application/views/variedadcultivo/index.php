    ...
    <div class="col-xs-12">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Variedad de cultivos</h3>
            </div>
...
<script type="text/javascript">
    var titles={'addtitle':"Nueva variedad de cultivo" ,'updatetitle':"Editar variedad de cultivo"};
    var isEdit=null,idEdit=null;
    $(document).ready(function () {

        var rowSelection = $('#tabla_grid').DataTable({
            "ajax": url_base+'variedadcultivo/getDataTable',
            "columns": [
                { "data": null },
    ...
    function btnSave(id){
        ...
        if(bol){
            ...
            $.post(url_base+'variedadcultivo/setForm',dataForm,function (data) {
                console.log(data);
                if(data.status === "ok"){
    ...
    function  editar(id){
        ...
        var idd=parseInt(id);
        $.post(url_base+'variedadcultivo/getData','id='+idd,function (data) {
            console.log(data);
            setForm(data[0]);
    ...
    function setForm(data){
        ...
        if(valData > 0){
            ...
            $("#isEdit").val(1);
            $("#idEdit").val(data.idvariedadcultivo);
        }else{
            alert_error("Error");
        }
    }
    ...
    function eliminar(id) {
        if(confirm('¿Esta seguro de eliminar este registro?')){
            var idd=parseInt(id);
            $.post(url_base+'variedadcultivo/deleteData','id='+idd,function (data) {
                if(data.status == 'ok'){
