...
<script type="text/javascript">
    ...
    var conceptoKardex=<?=$conceptocaja?>;
    ...
    function btnSave(){
        ...
        if(!($("#cliente").val()).trim()){alert_error("El cliente es requerido");return 0;}
...
<script type="text/template" id="tmpSelProductoVenta" >
    <select id="selProducto" name="selProducto" class="form-control" >
      ...
      <% _.each(data,function(i,k){ %>
        <option value="<%= i.idproductokardex+"-"+i.stock %>" ><%=i.cultivo+"-"+i.clasecultivo+"-"+i.catcultivo+" | "+i.cultivar +" ("+i.stock+"kg)" %></option>
        <%  }); %>
    </select>
</script>