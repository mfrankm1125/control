...
$gaArea=0;
$areasGa="";
$tableAreasGa='<table style="margin-top: 10px; border: 1px solid #ddd; border-collapse: collapse;width:600px;" border="1"   >
            <tr>
                <th style="width: 30px">Nro</th>
                <th style="width: 400px">Area</th>
                <th style="width: 70px"> Grado avance %</th>
...
foreach ($dataR as $dataArea) {
    ...
    $html2 .= '<h1 style="text-align: center;color: black;font-size: 20px;"><b><u>'.mb_strtoupper($dataArea["nombrearearesponsable"])  .'</u></b></h1>';
     ...
     $html2 .= '<p style="font-size: 15px"><b>Unidad Orgánica: <u>' .$dataArea["nombrearearesponsable"]  . '</u> - '.floatval(round($dataArea["DataArea"]["GaArea"]*100,2)).' %</b></p>';
        ...
        $tableAreasGa.='<tr>
                <td>'.$countareas.'</td>
                <td>' .$dataArea["nombrearearesponsable"]  . '</td>
                <td>'.floatval(round($dataArea["DataArea"]["GaArea"]*100,2)).' %</td>
                <td>'. $calificacionArea.'</td>
               </tr>';
        $cantidadActOp = 0;
    $sumatotalActOp = 0;
    foreach ($dataArea["DataArea"]["dataArea"] as $i) {
        ...
        foreach ($i["dataactpre"] as $ii) {
            ...
            foreach ($ii["dataactop"] as $iii) {
                ...
                $html2 .= '<b>Actividad Operativa: '. $iii["actop"] ." - ".$calificacion1."</b>" ;
                ...
                foreach ($iii["datatarea"] as $kk => $j) {
                    ...
                    $html2 .= '<td style="text-align: justify;text-justify: inter-word;width: 500px;padding: 2px;" >' . $j["tarea"] . '</td>';
                    $html2 .= '<td style="text-align: center;width: 40px;padding: 2px;" >' . $j["um"] . '</td>';
...
$html2 .='<p><u>Justificaciones:</u></p>';
   foreach($iii["datatarea"] as  $keyTarea=>$valTarea ){
    ...
    foreach ($valTarea["meses"]  as $valMeses){
        if(sizeof($valMeses["datajustify"]) > 0){
            ...
            foreach ($valMeses["datajustify"] as $valJustify){
                $html2 .="<p><b>Tarea ".($keyTarea+1)."</b> (".$mes[(int)$valMeses["id_tiempo"]]."): ".$valJustify["motivo"]."</p>";
                ...
                $html2 .= '
                    ...
                    <p class="centeredText">
                        <chart type="column" data-label-number="value" label-position-negative="inside" label-position="center"  style="width: 10cm;">
                            <title  font-size="9pt">Actividad Operativa: '.$iii["actop"].'</title>
        ...
        $html2.="<p style='font-size: 14px'>La ".$dataArea["nombrearearesponsable"]." luego de realizar sus actividades programadas para esta evaluación, obtuvo un grado de avance de <b>".floatval(round($dataArea["DataArea"]["GaArea"]*100,2))." %</b>, el que es calificado como ".$calificacion1." según el Tablero de medición.</p>";
    }
...
if($countareas > 1){
    if($countareas > 0 && $gaArea > 0){
        ...
        $html2.='<h1 style="color:black;font-size: 20px">[ Avance Global del POI: "'.$gaGlobalAreas.'% " - Calificación: ' .$calificacion.']</h1>';
    }
