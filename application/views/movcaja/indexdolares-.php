...
<script type="text/javascript">
                ...
                {   "sortable": false,
                    ...
                    "render": function ( data, type, full, meta ) {
                       ...
                       // html=html+"&nbsp; <a href='javascript:void(0)' onclick='editar("+id +","+JSON.stringify(full)+");'  class='btn btn-mint  btn-xs'><i class='fa fa-file'></i> Editar</a>";
                        ...
                        html=html+'&nbsp; <a href="javascript:void(0)" onclick="eliminar('+id +',\''+tablareferencia+'\',\''+idreferencia+'\');"  class="btn btn-danger btn-icon btn-xs"><i class="demo-psi-recycling icon-xs"></i> Eliminar</a>';
                        return html;
                    }
                }
            ],
    ...
    function eliminar(id,tabla,idx) {
        if(!confirm("¿Seguro de eliminar este registro?")){
            return 0;
        }
        $.post(url_base+"Movcajadolares/delete",{"id":id,tabla:tabla,idref:idx},function (data) {
            if(data.status == "ok"){
                refrescar();
                alert_success("Correcto");
